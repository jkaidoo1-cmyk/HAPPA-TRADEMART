...
const SELF_USER_STRING_FIELDS = ['sub_paid_at'];
...
function scrubPII(table, rec) {
  ...
  if (table === 'users') {
    ...
    drop(['email', 'phone', 'wallet_balance', 'id_image', 'proof_sales_1', 'proof_sales_2',
      ...
      'sub_payment_status', 'sub_payment_months', 'sub_payment_amount', 'sub_paid_at']);
...
function sanitizeUserCreate(body) {
  ...
  // Coerced fields (role/status/wallet/is_verified/id_verified) are reapplied
  // AFTER stripping admin-only fields, since they live in that list but the
  // user may still legitimately sign up as buyer/vendor/rendor.
