...
<div class="package-card" onclick="showPackageDetailModal('${pkg.id}')">
  ...
  <div class="package-body">
    ...
    <div style="font-size:.84rem;margin-bottom:8px;font-weight:500">
      ${(pkg.items||[]).slice(0,2).map(i=>{const im=itemImage(i);return `<div style="display:flex;justify-content:space-between;align-items:center;gap:8px;padding:2px 0"><span style="display:flex;align-items:center;gap:8px;min-width:0">${im ? `<img src="${im}" style="width:30px;height:30px;border-radius:6px;object-fit:contain;border:1px solid var(--border);flex-shrink:0;background:#fff" onerror="this.style.display='none'">` : ''}<span style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escHtml(i.name||'')} × ${i.qty}</span></span><span style="flex-shrink:0">GHS ${(i.price*i.qty).toFixed(2)}</span></div>`;}).join('')}
    ...
    <div class="card-body" style="padding:12px">
      ...
      ${(pkg.items||[]).map(i=>{
        ...
        <div style="padding:8px 0;border-bottom:1px solid var(--border);display:flex;gap:10px;align-items:center">
          <img src="${img}" alt="${title}" style="width:38px;height:38px;object-fit:contain;border-radius:6px;border:1px solid var(--border);flex-shrink:0;background:#fff" onerror="this.src='https://via.placeholder.com/50x50?text=Item'">
    ...
    <div style="margin-bottom:10px;border:1px solid var(--border);border-radius:var(--radius-sm);overflow:hidden">
      ...
      ${(pkg.items||[]).map(i=>{
        ...
        <div style="padding:8px 10px;border-bottom:1px solid var(--border);display:flex;gap:10px;align-items:center;background:#fff">
          <img src="${itemImg}" alt="${itemTitle}" style="width:42px;height:42px;object-fit:contain;border-radius:6px;border:1px solid var(--border);flex-shrink:0;background:#fff" onerror="this.src='https://via.placeholder.com/60x60?text=Item'">
          <div style="flex:1;min-width:0">
