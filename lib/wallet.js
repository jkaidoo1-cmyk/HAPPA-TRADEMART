...
async function rendorSubscribe(adapter, viewer, body) {
  ...
  let expected = Number.isFinite(override) && override > 0 ? override : parseFloat(await adapter.getSetting('rendor_sub_price', ''));
  if (!Number.isFinite(expected) || expected <= 0) expected = 30;
  expected = r2(expected);
  if (Math.round(amount * 100) !== Math.round(expected * 100)) {
    return fail(400, `The subscription costs GHS ${expected.toFixed(2)}.`);
  }
