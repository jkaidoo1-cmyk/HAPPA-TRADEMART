...
async function showSearchSuggestions(q) {
  ...
  // Search stores by name + category + keywords
  const matchedStores = App.allStores.filter(s =>
    isStoreVisibleOnMain(s) && shouldShowStoreOnMainWebsite(s) && (
      ...
      (Array.isArray(s.keywords) && s.keywords.some(k => k.toLowerCase().includes(ql)))
    )
  ).slice(0, 3);
  ...
  if (matchedProducts.length) {
    ...
    <div class="search-suggestion" onclick="openProduct('${p.id}');hideSearchDropdown()">
      <img src="${p.images?.[0]||'https://via.placeholder.com/30x30?text=P'}" style="width:30px;height:30px;border-radius:4px;object-fit:contain;background:#f8f9fa" onerror="this.src='https://via.placeholder.com/30x30?text=P'">
