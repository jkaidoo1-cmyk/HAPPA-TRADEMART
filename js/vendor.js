    ...
    <div class="card" style="margin-top:14px">
      ...
      <div class="card-body" style="padding:0">
        ...
        <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-bottom:1px solid var(--border)">
          ...
          <img src="${p.images?.[0]||'https://via.placeholder.com/40x40?text=P'}" style="width:36px;height:36px;border-radius:6px;object-fit:contain;background:#fff" onerror="this.src='https://via.placeholder.com/40x40?text=P'">
        ...
        <div class="storefront-split-wrap" style="display:flex;gap:20px;flex-wrap:wrap;align-items:flex-start${myStorefront.status === 'pending_approval' ? ';opacity:.7;pointer-events:none' : ''}">
          ...
          <div style="flex:1;min-width:300px;position:sticky;top:80px;align-self:start">
            ...
            <div class="card" style="overflow:hidden;border-radius:16px;border:1px solid var(--border);box-shadow:var(--shadow-md)">
              ...
              <div class="card-body" style="padding:0;background:#fff;max-height:480px;position:relative;overflow-y:scroll !important" id="storefront-live-preview-box">
...
<div class="card" style="margin-bottom:10px">
  <div class="card-body" style="display:flex;gap:10px;align-items:flex-start">
    <img src="${p.images?.[0]||'https://via.placeholder.com/70x70?text=P'}" style="width:64px;height:64px;border-radius:var(--radius-sm);object-fit:contain;background:#fff;flex-shrink:0"
...
<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--border)">
  ${rowImg ? `<img src="${rowImg}" style="width:40px;height:40px;border-radius:8px;object-fit:contain;border:1px solid var(--border);flex-shrink:0;background:#fff" onerror="this.style.display='none'">` : ''}
...
function showAddProductModal(storeId, vendorId) {
  ...
  let store = App.allStores.find(s => String(s.id) === String(storeId));
  if (!store && vendorId) {
    store = App.allStores.find(s => String(s.vendor_id) === String(vendorId));
    if (store) storeId = store.id;
  }
  if (!store) {
    showToast('Please set up your store before adding products.', 'warning');
    return;
  }
  ...
  showModal(_addProductModalHTML(storeId, vendorId, {}));
...
function _addProductImgSlot(prefix) {
  ...
  slot.innerHTML = `
    <img id="${thumbId}" style="width:100%;height:100%;object-fit:contain;background:#fff;display:none">
...
async function _onProductImgPick(input, prefix, idx) {
  ...
  if (file.size > maxBytes) { showToast('Image too large. Max 5MB.', 'warning'); input.value = ''; return; }
  
  try {
    const base64 = await squareImage(file, 900, 0.72);
...
async function submitAddProduct(e, storeId, vendorId) {
  ...
  const setBtn = OptimisticUI.button(submitBtn, '<i class="fas fa-plus-circle"></i> Add Product');
  setBtn('saving');

  const resetBtn = () => setBtn('idle');
  ...
  if (name && cat && desc && typeof window.localLearnCorrection === 'function') {
    try { window.localLearnCorrection(name, cat, desc); } catch(_) {}
  }
  ...
  // Optimistic: insert temp product immediately so UI feels instant
  ...
  setTimeout(() => {
    const el = document.querySelector(`[data-prod-id="${tempId}"]`);
    if (el) OptimisticUI.fadeIn(el);
  }, 50);

  try {
    const prod = await apiPost('products', productData);
    if (prod) {
      // Replace temp with real product (preserves UI position)
      const idx = App.allProducts.findIndex(p => p.id === tempId);
      if (idx > -1) {
        App.allProducts[idx] = { ...prod, _isOptimistic: false };
        if (App.currentPage === 'vendor-my-store') {
          renderVendorMyStorePage();
        } else {
          renderVendorDashboard();
    ...
    App.allProducts = App.allProducts.filter(p => p.id !== tempId);
    if (App.currentPage === 'vendor-my-store') {
      renderVendorMyStorePage();
    } else {
      renderVendorDashboard();
...
// ── Helper: re-open the add-product modal with previous values
function _addProductModalHTML(storeId, vendorId, prev) {
  return `
...
function _rehydrateAddProductModal(prev, images) {
  // Re-add the image slots
  images.forEach((src, i) => {
    _addProductImgSlot('new-p');
    const hid = document.getElementById('new-p-img-b64-' + i);
    if (hid) hid.value = src;
    const thumb = document.getElementById('new-p-thumb-' + i);
    if (thumb) { thumb.src = src; thumb.style.display = 'block'; }
    const placeholder = document.getElementById('new-p-slot-placeholder-' + i);
    if (placeholder) placeholder.style.display = 'none';
  });
...
async function showEditProductModal(productId) {
  ...
  const existingSlotsHTML = existingImgs.map((src, i) => `
    <div class="pi-slot" id="edit-p-slot-${i}" style="position:relative;width:72px;height:72px;border-radius:8px;overflow:hidden;border:2px solid var(--border);background:var(--bg);flex-shrink:0">
      <img id="edit-p-thumb-${i}" src="${escHtml(src)}" style="width:100%;height:100%;object-fit:contain;background:#fff;display:block">
...
async function _bapOnFilePick(input) {
  ...
  for (const file of files) {
    ...
    _bap.drafts.push({ b64: '', name: '', desc: '', price: '', orig: '', stock: '', weight: '0.5', cat: '', flash: false, allowNote: false, notePrompt: '' });
...
<article class="bap-draft-card" id="bap-card-${i}">
  ...
  <div style="padding:8px 10px 0;border-top:1px solid var(--border)">
    ...
    <div id="bap-img-slots-${i}" style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:6px">
      ...
      <div class="pi-slot" id="bap-p-slot-0-${i}" style="position:relative;width:52px;height:52px;border-radius:6px;overflow:hidden;border:2px solid var(--primary);background:var(--bg);flex-shrink:0">
        <img id="bap-p-thumb-0-${i}" src="${d.b64}" style="width:100%;height:100%;object-fit:contain;background:#fff;display:block">
  ...
  <div class="bap-draft-form">
    ...
    <select class="form-control form-select" id="bap-cat-${i}" onchange="_bapHandleCategoryChange(${i},this.value)">
      <option value="" ${!d.cat ? 'selected' : ''}>Select category (optional)</option>
...
function _bapHandleDescInput(idx, desc) {
  ...
  const name = (document.getElementById(`bap-name-${idx}`)?.value || '').trim();
  const cat = document.getElementById(`bap-cat-${idx}`)?.value || '';
  if (name && typeof localLearnCorrection === 'function') {
    localLearnCorrection(name, cat, desc);
  }
}
...
function _bapAddSlot(cardIdx) {
  ...
  slot.innerHTML = `
    <img id="bap-p-thumb-${slotIdx}-${cardIdx}" style="width:100%;height:100%;object-fit:contain;background:#fff;display:none">
...
async function _bapSlotPick(input, cardIdx, slotIdx) {
  const file = input.files?.[0];
  if (!file) return;
  ...
  try {
    ...
    if (slotIdx === 0 && _bap.drafts[cardIdx]) {
      _bap.drafts[cardIdx].b64 = base64;
...
function _bapRemoveSlot(cardIdx, slotIdx) {
  ...
  if (container) {
    container.querySelectorAll('.pi-slot').forEach((s, i) => {
      ...
      if (i === 0) {
        if (!s.querySelector('span[style*="bottom:1px"]')) {
          const b = document.createElement('span');
...
async function _bapSubmitAll() {
  ...
  if (store && store.id) {
    ...
    if (active.length > remainingSlotCount) {
      showToast(`Your plan only allows ${remainingSlotCount} more product(s) (Limit: ${maxAllowed}). Please remove ${active.length - remainingSlotCount} item(s) or upgrade your plan.`, 'warning', 5000);
      return;
    }
  }
...
window.updateStorefrontPreview = function() {
  ...
  const getProductImageHTML = (p) => {
    if (p.image_url && (p.image_url.startsWith('http') || p.image_url.startsWith('data:') || p.image_url.includes('/') || p.image_url.includes('.'))) {
      return `<img src="${p.image_url}" style="width:100%; height:100%; object-fit:contain;background:#fff" onerror="this.outerHTML='📦'">`;
    }
    return p.image_url || '📦';
  };
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px">
          ${displayProducts.slice(0, 4).map(p => `
            <div class="product-card">
              <div class="product-img" style="height:60px; display:flex; align-items:center; justify-content:center; font-size:1.5rem; background:#fff">
      ...
      <div style="padding:10px">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:6px">
          ${displayProducts.slice(0, 2).map(p => `
            <div class="product-card">
              <div class="product-img" style="height:50px; display:flex; align-items:center; justify-content:center; font-size:1.2rem; background:#fff">
                ${getProductImageHTML(p)}
              </div>
