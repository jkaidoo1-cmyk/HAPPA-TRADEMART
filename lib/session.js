...
const THIRTY_DAYS_MS = 365 * 24 * 60 * 60 * 1000; // 1 year — sliding refresh keeps active sessions alive indefinitely
const inMemorySessions = new Map(); // token -> { userId, role, expiresAt }

function resolvedSessionSecretPath() {
  return path.join(__dirname, '..', '.session-secret');
}
...
function loadOrCreateSessionSecret() {
  ...
  const secretPath = resolvedSessionSecretPath();
  try {
    ...
    console.warn('[Session] Failed to persist a fallback secret; using in-memory mode for this process only.', error.message);
    return '';
  }
}
...
function sessionSecret() {
  const configured = String(process.env.SESSION_SECRET || '').trim();
  if (configured) return configured;
  return loadOrCreateSessionSecret();
}
...
function getSessionUser(req) {
  ...
  if (secret) {
    ...
    if (parts.length !== 2) {
      const legacySession = inMemorySessions.get(token);
      if (!legacySession) return null;
      if (Date.now() > legacySession.expiresAt) {
        inMemorySessions.delete(token);
        return null;
      }
      return { userId: legacySession.userId, role: legacySession.role, token };
    }
...
/**
 * True when the request presents an Authorization token that fails
 * validation — i.e. a dead/expired/unknown credential (backend restart,
 * serverless instance rotation, forged token). Servers use this to answer
 * 401 instead of silently treating the caller as anonymous, which would make
 * owner-only tables (notifications, wallet, …) return empty lists for an
 * expired session and freeze the client until re-login.
 */
function hasInvalidSession(req) {
  const authHeader = req.headers['authorization'] || '';
